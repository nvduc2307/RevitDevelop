...
namespace RevitDevelop.Utils.CadExtend;
public static class CadExtend
{
    ...
    public static XYZ PickPoint()
    {
        try
        {
            ...
            arr = ((IEnumerable)pointCad).Cast<object>()
                    .Select(x => x.ToString())
                    .ToArray();
            return new XYZ(double.Parse(arr[0]), double.Parse(arr[1]), 0);
        }
        catch
        {
            return null;
    ...
    public static IList<string> GetAllText(string acadDoc, string layer)
    {
        IList<string> texts = [];
        dynamic acadApp;
        try
        ...
        foreach (dynamic doc in acadDocs)
        {
            ...
            if (name == acadDoc)
            {
                ...
                // Access the Model Space block table record
                dynamic modelSpace = db.ModelSpace;

                // Loop through each entity in the Model Space
                foreach (dynamic entity in modelSpace)
                {
                    // Check if the entity is of type DBText (single-line text)
                    if (entity.Layer == layer)
                    {
                        if (entity.EntityName == "AcDbText" || entity.EntityName == "AcDbMText")
                        {
                            string textValue = entity.TextString;
                            texts.Add(textValue);
                        }
                    }
                }
